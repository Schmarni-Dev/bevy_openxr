...
use input::XrInput;
use openxr as xr;
use passthrough::{supports_passthrough, XrPassthroughLayer};
...
impl Plugin for OpenXrPlugin {
    ...
    fn finish(&self, app: &mut App) {
        ...
        if let Some(data) = app.world.get_resource::<XrRenderData>().cloned() {
            ...
            if passthrough {
                info!("Passthrough!");
                if let Ok(passthrough) = data
                    .xr_session
                    .create_passthrough(xr::PassthroughFlagsFB::IS_RUNNING_AT_CREATION)
                {
                    if let Ok(passthrough_layer) = data.xr_session.create_passthrough_layer(
                        &passthrough,
                        xr::PassthroughFlagsFB::IS_RUNNING_AT_CREATION,
                        xr::PassthroughLayerPurposeFB::RECONSTRUCTION,
                    ) {
                        app.insert_resource(passthrough_layer);
                    }
                    app.insert_resource(passthrough);
                }
            }
